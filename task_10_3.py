...
class Cell:
    ...
    def __add__(self, other):
        return f'Сумма 2х клеток: {self.quantity + other.quantity}'

    def __sub__(self, other):
        result = self.quantity - other.quantity
        if result > 0:
            return f'Разность 2х клеток: {result}'
        else:
            return f'Клеток нет'

    def __mul__(self, other):
        return f'Новая клетка после умножения: {self.quantity * other.quantity}'

    def __truediv__(self, other):
        return f'Новая клетка после деления: {self.quantity // other.quantity}'
